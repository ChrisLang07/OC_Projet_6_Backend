    ...
    Book.findById(bookId)
      .then(book => {
        ...
        if (req.file) {
          ...
          const tempOptimizedImagePath = path.join(__dirname, '../images', `${uuidv4()}.webp`); // Chemin pour l'image optimisée
          ...
          sharp(tempImagePath)
            ...
            .toFile(tempOptimizedImagePath, (err, info) => {
              if (err) {
                ...
                return res.status(500).json({ error: 'Erreur lors de l\'optimisation de l\'image.', details: err.message });
              }
              console.log('Optimisation réussie :', info);
  
              // Déplacer l'image optimisée à son emplacement final
              const finalImagePath = path.join(__dirname, '../images', req.file.filename);
              fs.rename(tempOptimizedImagePath, finalImagePath, (err) => {
                if (err) {
                  console.error('Erreur lors du déplacement de l\'image optimisée :', err);
                  return res.status(500).json({ error: 'Erreur lors du déplacement de l\'image optimisée.', details: err.message });
                }
      ...
      .catch(error => res.status(500).json({ error: 'Erreur lors de la récupération du livre.', details: error }));
  };

exports.deleteBook = (req, res, next) => {
  const bookId = req.params.id;
  
  // On commence par récupérer le livre et son image
  Book.findById(bookId)
    .then(book => {
      if (!book) {
        return res.status(404).json({ message: 'Livre non trouvé !' });
      ...
      // Extraire le chemin de l'image à partir de l'URL stockée
      ...
      fs.unlink(imagePath, (err) => {
        if (err) {
          return res.status(500).json({ error: 'Erreur lors de la suppression de l\'image.', details: err });
        }

        // Après avoir supprimé l'image, on supprime le livre de la base de données
        Book.deleteOne({ _id: bookId })
          .then(() => res.status(200).json({ message: 'Livre et image supprimés !' }))
          .catch(error => res.status(500).json({ error: 'Erreur lors de la suppression du livre.', details: error }));
      });
    })
    .catch(error => res.status(500).json({ error: 'Erreur lors de la récupération du livre.', details: error }));
};
